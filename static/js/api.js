class ExamAPI {
    static async getExams() {
        try {
            const response = await fetch('http://localhost:3000/getExams');
        ...
        try {
            const response = await fetch(`http://localhost:3000/examsBank/${filename}`);
    ...
    static async saveExamResult(examId, examResult) {
        try {
            const response = await fetch('http://localhost:3000/saveExamResult', {
                method: 'POST',
